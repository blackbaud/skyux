...
- Additions to the public API must be:
  ...
  - **Named with `sky`** — the name must carry `sky`/`Sky`, normally as the
    prefix (`skyDoThing`, `SkyThingOptions`, `SkyThingResult`), but it may sit
    inside the name when a framework convention owns the prefix
...
Two kinds of file never need a spec of their own: a barrel that only re-exports
other files (`src/index.ts`, `testing/src/public-api.ts`), and a file whose
header marks it `AUTO-GENERATED` (e.g.
[libs/components/code-examples/routes/src/index.ts](libs/components/code-examples/routes/src/index.ts),
regenerated by `npx nx run code-examples:prebuild`). Both are covered by the
specs of the files they point at.
...
- Distinguish blocking issues (public API breaks, new heavy deps, a barrel
  export whose name omits `sky`, missing tests, coverage regressions) from
