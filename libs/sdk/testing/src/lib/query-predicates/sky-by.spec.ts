...
describe('SkyBy', () => {
  ...
  describe('skyId', () => {
    it('should find an element with the appropriate data-sky-id attribute if it exists', () => {
      ...
      const elementWithDataSkyId: DebugElement = fixture.debugElement.query(SkyBy.skyId('my-sky-id'));

      expect(elementWithDataSkyId.nativeElement).toExist();
      expect(elementWithDataSkyId.nativeElement.getAttribute('data-sky-id')).toEqual('my-sky-id');
    })
    ...
    it('should not find an element with a data-sky-id that does not exist', () => {
      ...
      expect(fixture.debugElement.query(SkyBy.skyId('different-sky-id'))).not.toExist();
    });
  });
});
