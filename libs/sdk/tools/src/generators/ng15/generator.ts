        ...
        if ('peerDependencies' in json) {
          ...
          for (const [name, version] of Object.entries(peerDependencies)) {
            if (name in rootPackageJsonDependencies) {
              let rootVersion = rootPackageJsonDependencies[name];
              if (rootVersion.match(/^[0-9]+\.[0-9]+\.[0-9]+$/)) {
                rootVersion = `^${rootVersion}`;
              }
              if (`${version}`.match(/^[^][0-9]+\.[0-9]+\.[0-9]+$/)) {
                peerDependencies[name] = rootVersion;
              } else if (!semver.satisfies(rootPackageJsonDependencies[name], `${version}`)) {
                throw new Error(
                  `The version of ${name} in ${projectConfig.root} is not compatible with the version ${rootVersion} from the root package.json.`
                );
              }
            }
          }
