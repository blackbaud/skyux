...
describe('HelpInitializationService', () => {
  ...
  it('should call BBHelpClient.load with svcid from SkyAppConfig', fakeAsync(() => {
    ...
    const initializationService = new HelpInitializationService(
      buildWindow(),
      buildConfig({ svcid: svcid })
    );
    const givenConfig: HelpWidgetConfig = { productId: 'test_id' };
    const expectedConfig: HelpWidgetConfig = {
      ...givenConfig,
      ...{ extends: svcid },
    };
    initializationService.load(givenConfig);
  ...
  it("should call BBHelpClient.load with overwritten 'extends' value from svcid", fakeAsync(() => {
    ...
    const givenConfig: HelpWidgetConfig = {
      productId: 'test_id',
      extends: 'extends',
    };
    const expectedConfig: HelpWidgetConfig = {
  ...
  it('should call BBHelpClient.load with empty locale when undefined value in SKYUX_HOST', fakeAsync(() => {
    const window: SkyAppWindowRef = {
      nativeWindow: { SKYUX_HOST: { acceptLanguage: undefined } },
    } as SkyAppWindowRef;
    const initializationService = new HelpInitializationService(
      window,
      buildConfig()
  ...
  it('should call BBHelpClient.load with config from given extension', fakeAsync(() => {
    ...
    const expectedConfig: HelpWidgetConfig = {
      caseCentralUrl: 'https://case.centr.al',
    };
...
function buildConfig(
  params: { svcid?: string; envid?: string } = undefined
): SkyAppConfig {
  const host = 'https://example.com';
  const url: string = params
    ? `${host}?${buildQueryParamString(params)}`
    : host;
  const runtimeParams: SkyAppRuntimeConfigParams =
    new SkyAppRuntimeConfigParams(url, { svcid: true, envid: true });
