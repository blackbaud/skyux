...
import { getSkyuxLineChartConfig } from './line-chart-config';
...
export function transformToChartJsConfig(
  ...
  // prettier-ignore
  const chartOptions = getSkyuxLineChartConfig({
...
): Partial<ChartOptions<'line'>['scales']> {
  const scales: Partial<ChartOptions<'line'>['scales']> = {};
