...
describe('Auth interceptor', () => {
  ...
  beforeEach(() => {
    mockTokenProvider = jasmine.createSpyObj('SkyAuthTokenProvider', [
      'getContextToken',
      'decodeToken',
    ]);
  ...
  it('should add the default permission scope if it is injected and a scope is not passed in', (done) => {
  ...
  it('should not add the default permission scope if it is not injected', (done) => {
    const interceptor = TestBed.inject(SkyAuthInterceptor);
    const request = createRequest(true);
  ...
  it('should not add the default permission scope if it is injected and a scope is passed in', (done) => {
    TestBed.configureTestingModule({
      providers: [
        {
          provide: SKY_AUTH_DEFAULT_PERMISSION_SCOPE,
