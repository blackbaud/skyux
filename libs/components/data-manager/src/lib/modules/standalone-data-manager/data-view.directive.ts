...
export class SkyDataViewDirective {
  ...
  public readonly isActive = output<boolean>();
  ...
  readonly #dataViewsUpdates = toSignal(
    this.#viewIdObs.pipe(
      filter((id): id is string => !!id),
      switchMap((id) => this.#dataManagerService.getDataStateUpdates(id)),
    ),
  );
  ...
  constructor() {
    ...
    this.#dataManagerService
      .getDataStateUpdates(`dataViewSearchText`, { properties: ['searchText'] })
