...
export class SkyConfirmComponent implements OnDestroy {
  ...
  protected buttons: Signal<SkyConfirmButton[] | undefined>;
  ...
  constructor() {
    ...
    if (
      ...
      this.buttons = signal(
        this.#getCustomButtons(this.#config.buttons),
      ).asReadonly();
    } else {
      this.buttons = toSignal(this.#getPresetButtons());
    }
