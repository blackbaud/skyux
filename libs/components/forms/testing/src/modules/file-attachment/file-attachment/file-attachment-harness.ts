...
type TestElementDataTransfer = DataTransfer & { [key: string]: EventData };
...
export class SkyFileAttachmentHarness extends SkyComponentHarness {
  ...
  async #dropFile(file: File): Promise<void> {
    ...
    const dataTransfer = new DataTransfer() as TestElementDataTransfer;
    dataTransfer.items.add(file);

    await uploadLocation.dispatchEvent('drop', {
      dataTransfer,
    });
  }
